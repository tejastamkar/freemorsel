...
export const donationComplete = functions
  .region("asia-south1")
  .firestore.document("Donation/{donationId}")
  .onCreate(async (snapshot, context) => {
    const donationData = snapshot.data();

    var userDetail = await db
      .doc(`Users/${donationData["userId"]}`)
      .get()
    ...
    const payload = {
      ...
      notification: {
        title: `FreeMorsel`,
        body: `Your Donation is done successfully`,
      },
      data: {
...
export const pickupCall = functions
  .region("asia-south1")
  .firestore.document("PendingDonation/{donationId}")
  .onCreate(async (snapshot, context) => {
    ...
    await db
      ...
      .get()
      .then((value) =>
        value.docs.forEach(async (riderdetail) => {
          const payload = {
...
export const pickupAlertCall = functions
  .region("asia-south1")
  .firestore.document("PendingDonation/{donationId}")
        ...
        .then((value) =>
          value.docs.forEach(async (doc) => {
            const payload = {
              ...
              notification: {
                title: `FreeMorsel`,
                body: `Your order will be picked up by ${
                  doc.data()["username"]
                }`,
              },
              data: {
